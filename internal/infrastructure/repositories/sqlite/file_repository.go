...
func (r *FileRepository) GetByHash(ctx context.Context, hash string) ([]*entities.File, error) {
	...
	for rows.Next() {
		...
		// Parse time strings
		modifiedTime, err := parseTimeString(modifiedTimeStr)
		if err != nil {
			return nil, fmt.Errorf("failed to parse modified_time: %v", err)
		}
		file.ModifiedTime = modifiedTime

		lastUpdated, err := parseTimeString(lastUpdatedStr)
		if err != nil {
			return nil, fmt.Errorf("failed to parse last_updated: %v", err)
		}
		file.LastUpdated = lastUpdated
...
func (r *FileRepository) FindFilesWithoutHash(ctx context.Context, limit int) ([]*entities.File, error) {
	// 1GB 제한을 고려하여 해시 계산 가능한 파일만 선택
	// 큰 파일보다 작은 파일부터 처리하여 성공률 높임
	maxHashableSize := int64(1024 * 1024 * 1024) // 1GB
	...
	ORDER BY size ASC, modified_time ASC
	LIMIT ?
	`

	rows, err := r.db.QueryContext(ctx, query, maxHashableSize, limit)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
func (r *FileRepository) GetStatistics(ctx context.Context) (*entities.FileStatistics, error) {
	...
	// Get hash statistics
	query = `SELECT COUNT(*), COALESCE(SUM(size), 0) FROM files WHERE hash IS NOT NULL AND hash != '' AND hash_calculated = TRUE`
	err = r.db.QueryRowContext(ctx, query).Scan(&stats.HashedFiles, &stats.HashedSize)
	if err != nil {
		return nil, err
	}

	// Calculate hashed percentage
	if stats.TotalFiles > 0 {
		stats.HashedPercentage = float64(stats.HashedFiles) / float64(stats.TotalFiles) * 100
	}
...
func (r *FileRepository) scanFile(rows *sql.Rows) (*entities.File, error) {
	...
	// Parse modified time
	if modifiedTimeStr.Valid && modifiedTimeStr.String != "" {
		if modifiedTime, err := parseTimeString(modifiedTimeStr.String); err == nil {
			file.ModifiedTime = modifiedTime
		}
	}
	...
	// Parse last updated time
	if lastUpdatedStr.Valid && lastUpdatedStr.String != "" {
		if lastUpdated, err := parseTimeString(lastUpdatedStr.String); err == nil {
			file.LastUpdated = lastUpdated
		} else {
			file.LastUpdated = time.Now()
		}
...
// parseTimeString attempts to parse time strings in various formats
func parseTimeString(timeStr string) (time.Time, error) {
	...
	return time.Now(), fmt.Errorf("unable to parse time string: %s", timeStr)
}
