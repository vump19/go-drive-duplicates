...
func (r *DuplicateGroupRepository) Save(ctx context.Context, group *entities.DuplicateGroup) error {
	...
	// UPSERT: Check if group with this hash already exists
	var existingID int
	err = tx.GetContext(ctx, &existingID, "SELECT id FROM duplicate_groups WHERE hash = ?", group.Hash)
	...
	if err == nil {
		...
		SET count = ?, total_size = ?, updated_at = ?
		WHERE id = ?
		`
		_, err := tx.ExecContext(ctx, query, group.Count, group.TotalSize, group.UpdatedAt, group.ID)
		if err != nil {
			return err
		}
		...
		// Group doesn't exist, insert new
		query := `
...
func (r *DuplicateGroupRepository) FindByID(ctx context.Context, id int) (*entities.DuplicateGroup, error) {
	query := `
	SELECT id, hash, count, total_size, created_at, updated_at, COALESCE(memo, '') as memo
	...
	err := r.db.QueryRowContext(ctx, query, id).Scan(
		...
		&createdAtStr, &updatedAtStr, &group.Memo,
	)
...
func (r *DuplicateGroupRepository) Delete(ctx context.Context, id int) error {
	log.Printf("🗑️ 중복 그룹 삭제 시작: 그룹 ID %d", id)

	// First disable foreign key constraints globally (outside transaction)
	_, err := r.db.ExecContext(ctx, "PRAGMA foreign_keys = OFF")
	if err != nil {
		log.Printf("⚠️ 전역 외래 키 제약 조건 비활성화 실패: %v", err)
	} else {
		log.Printf("🔓 전역 외래 키 제약 조건 비활성화")
	}

	// Ensure we re-enable foreign keys when done
	defer func() {
		_, err := r.db.ExecContext(ctx, "PRAGMA foreign_keys = ON")
		if err != nil {
			log.Printf("⚠️ 전역 외래 키 제약 조건 재활성화 실패: %v", err)
		} else {
			log.Printf("🔒 전역 외래 키 제약 조건 재활성화")
		}
	}()

	// Check what references exist for this group
	var fileCount int
	err = r.db.QueryRowContext(ctx, "SELECT COUNT(*) FROM duplicate_group_files WHERE group_id = ?", id).Scan(&fileCount)
	if err != nil {
		return err
	}
	log.Printf("🔍 그룹 ID %d의 파일 관계 수: %d", id, fileCount)

	// Now start transaction for the actual deletions
	tx, err := r.db.BeginTxx(ctx, nil)
	if err != nil {
		return err
	}
	...
	// 1. Delete comparison_duplicate_files that reference files in this group (if table exists)
	var tableExists int
	err = tx.QueryRowContext(ctx, "SELECT COUNT(*) FROM sqlite_master WHERE type='table' AND name='comparison_duplicate_files'").Scan(&tableExists)
	if err == nil && tableExists > 0 {
		...
		if err == nil {
			...
			log.Printf("⚠️ comparison_duplicate_files 삭제 실패 (무시함): %v", err)
		}
	}
	...
	log.Printf("✅ 중복 그룹 삭제 성공: 그룹 ID %d", id)
	return nil
}
...
func (r *DuplicateGroupRepository) GetValidGroups(ctx context.Context) ([]*entities.DuplicateGroup, error) {
	query := `
	SELECT dg.id, dg.hash, dg.count, dg.total_size, dg.created_at, dg.updated_at, COALESCE(dg.memo, '') as memo
	FROM duplicate_groups dg
	WHERE dg.count > 1
...
// UpdateMemo updates the memo field for a duplicate group
func (r *DuplicateGroupRepository) UpdateMemo(ctx context.Context, id int, memo string) error {
	query := `UPDATE duplicate_groups SET memo = ?, updated_at = ? WHERE id = ?`
	result, err := r.db.ExecContext(ctx, query, memo, time.Now(), id)
	if err != nil {
		return err
	}
	rowsAffected, err := result.RowsAffected()
	...
	if rowsAffected == 0 {
		return fmt.Errorf("중복 그룹을 찾을 수 없습니다: %d", id)
	}
	return nil
}
