...
func (r *ComparisonResultRepository) FindByID(ctx context.Context, id int) (*entities.ComparisonResult, error) {
	...
	// Load unique files
	uniqueInSource, err := r.LoadUniqueFiles(ctx, result.ID, "source")
	if err == nil {
		result.UniqueInSource = uniqueInSource
	}
	uniqueInTarget, err := r.LoadUniqueFiles(ctx, result.ID, "target")
	if err == nil {
		result.UniqueInTarget = uniqueInTarget
	}
...
// SaveUniqueFiles saves unique files for a comparison result
func (r *ComparisonResultRepository) SaveUniqueFiles(ctx context.Context, comparisonID int, files []*entities.File, folderType string) error {
	if len(files) == 0 {
		return nil
	}

	log.Printf("💾 비중복 파일 저장 시작 - comparisonID: %d, folderType: %s, 파일 수: %d", comparisonID, folderType, len(files))
	...
	// Delete existing unique files for this comparison and folder type
	deleteQuery := `DELETE FROM comparison_unique_files WHERE comparison_id = ? AND folder_type = ?`
	if _, err := tx.ExecContext(ctx, deleteQuery, comparisonID, folderType); err != nil {
		log.Printf("❌ 기존 비중복 파일 삭제 실패: %v", err)
		return err
	}

	// Insert unique files in batches
	const batchSize = 100
	for i := 0; i < len(files); i += batchSize {
		...
		batch := files[i:end]
		query := `INSERT INTO comparison_unique_files (comparison_id, file_id, file_name, file_path, file_size, folder_type) VALUES `
		values := make([]string, len(batch))
		args := make([]interface{}, len(batch)*6)

		for j, file := range batch {
			values[j] = "(?, ?, ?, ?, ?, ?)"
			args[j*6] = comparisonID
			args[j*6+1] = file.ID
			args[j*6+2] = file.Name
			args[j*6+3] = file.Path
			args[j*6+4] = file.Size
			args[j*6+5] = folderType
		}

		query += strings.Join(values, ", ")
		if _, err := tx.ExecContext(ctx, query, args...); err != nil {
			log.Printf("❌ 비중복 파일 배치 저장 실패: %v", err)
			return err
		}
	}

	if err := tx.Commit(); err != nil {
		log.Printf("❌ 비중복 파일 트랜잭션 커밋 실패: %v", err)
		return err
	}

	log.Printf("✅ 비중복 파일 저장 완료 - %d개 파일", len(files))
	return nil
}

// LoadUniqueFiles loads unique files for a comparison result
func (r *ComparisonResultRepository) LoadUniqueFiles(ctx context.Context, comparisonID int, folderType string) ([]*entities.File, error) {
	query := `
	SELECT cuf.file_id, cuf.file_name, cuf.file_path, cuf.file_size, cuf.moved, cuf.new_file_id,
		   COALESCE(f.mime_type, '') as mime_type,
		   COALESCE(f.web_view_link, '') as web_view_link,
		   COALESCE(f.hash, '') as hash
	FROM comparison_unique_files cuf
	LEFT JOIN files f ON cuf.file_id = f.id
	WHERE cuf.comparison_id = ? AND cuf.folder_type = ?
	ORDER BY cuf.file_size DESC
	`

	rows, err := r.db.QueryContext(ctx, query, comparisonID, folderType)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	...
	for rows.Next() {
		var file entities.File
		var moved bool
		var newFileID sql.NullString

		err := rows.Scan(
			&file.ID, &file.Name, &file.Path, &file.Size, &moved, &newFileID,
			&file.MimeType, &file.WebViewLink, &file.Hash,
		)
		if err != nil {
			return nil, err
		}
...
// GetUniqueFilesCount returns the count of unique files for a comparison
func (r *ComparisonResultRepository) GetUniqueFilesCount(ctx context.Context, comparisonID int, folderType string) (int, error) {
	query := `SELECT COUNT(*) FROM comparison_unique_files WHERE comparison_id = ? AND folder_type = ?`
	var count int
	err := r.db.QueryRowContext(ctx, query, comparisonID, folderType).Scan(&count)
	return count, err
}

// GetUnmovedUniqueFiles returns unique files that haven't been moved yet
func (r *ComparisonResultRepository) GetUnmovedUniqueFiles(ctx context.Context, comparisonID int) ([]*entities.File, error) {
	query := `
	SELECT cuf.file_id, cuf.file_name, cuf.file_path, cuf.file_size,
		   COALESCE(f.mime_type, '') as mime_type,
		   COALESCE(f.web_view_link, '') as web_view_link,
		   COALESCE(f.hash, '') as hash
	FROM comparison_unique_files cuf
	LEFT JOIN files f ON cuf.file_id = f.id
	WHERE cuf.comparison_id = ? AND cuf.folder_type = 'target' AND cuf.moved = FALSE
	ORDER BY cuf.file_size DESC
	`
...
// MarkFileAsMoved marks a unique file as moved
func (r *ComparisonResultRepository) MarkFileAsMoved(ctx context.Context, comparisonID int, fileID string, newFileID string) error {
	query := `
	UPDATE comparison_unique_files
	SET moved = TRUE, moved_at = CURRENT_TIMESTAMP, new_file_id = ?
	WHERE comparison_id = ? AND file_id = ?
	`
	_, err := r.db.ExecContext(ctx, query, newFileID, comparisonID, fileID)
	return err
}

// GetMovedFilesCount returns the count of moved files for a comparison
func (r *ComparisonResultRepository) GetMovedFilesCount(ctx context.Context, comparisonID int) (int, error) {
	query := `SELECT COUNT(*) FROM comparison_unique_files WHERE comparison_id = ? AND moved = TRUE`
	var count int
	err := r.db.QueryRowContext(ctx, query, comparisonID).Scan(&count)
	return count, err
}
