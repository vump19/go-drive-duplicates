...
type ComparisonResult struct {
	...
	// Unique files (files that exist only in one folder)
	UniqueInSource []*File `json:"uniqueInSource"` // Files only in source folder
	UniqueInTarget []*File `json:"uniqueInTarget"` // Files only in target folder (can be moved to source)
...
func NewComparisonResult(sourceFolderID, targetFolderID, sourceFolderName, targetFolderName string) *ComparisonResult {
	...
	return &ComparisonResult{
		...
		UniqueInSource:   make([]*File, 0),
		UniqueInTarget:   make([]*File, 0),
		CreatedAt:        now,
		UpdatedAt:        now,
	}
}
...
// AddUniqueInSource adds a unique file from source folder
func (cr *ComparisonResult) AddUniqueInSource(file *File) {
	cr.UniqueInSource = append(cr.UniqueInSource, file)
	cr.UpdatedAt = time.Now()
}

// AddUniqueInTarget adds a unique file from target folder
func (cr *ComparisonResult) AddUniqueInTarget(file *File) {
	cr.UniqueInTarget = append(cr.UniqueInTarget, file)
	cr.UpdatedAt = time.Now()
}

// GetUniqueInTargetCount returns the number of unique files in target
func (cr *ComparisonResult) GetUniqueInTargetCount() int {
	return len(cr.UniqueInTarget)
}

// GetUniqueInTargetSize returns the total size of unique files in target
func (cr *ComparisonResult) GetUniqueInTargetSize() int64 {
	var size int64
	for _, file := range cr.UniqueInTarget {
		size += file.Size
	}
	return size
}

// GetUniqueInTargetFileIDs returns the IDs of unique files in target
func (cr *ComparisonResult) GetUniqueInTargetFileIDs() []string {
	ids := make([]string, len(cr.UniqueInTarget))
	for i, file := range cr.UniqueInTarget {
		ids[i] = file.ID
	}
	return ids
}

// HasUniqueFilesInTarget returns true if there are unique files in target that can be moved
func (cr *ComparisonResult) HasUniqueFilesInTarget() bool {
	return len(cr.UniqueInTarget) > 0
}
